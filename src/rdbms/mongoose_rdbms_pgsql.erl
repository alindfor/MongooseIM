...
-spec escape_binary(mongoose_rdbms:pool(), binary()) -> binary().
escape_binary(_Pool, Bin) when is_binary(Bin) ->
    <<"E'\\\\x", (bin_to_hex:bin_to_hex(Bin))/binary, "'">>.

-spec unescape_binary(mongoose_rdbms:pool(), binary()) -> binary().
unescape_binary(_Pool, <<"E'\\\\x", Bin/binary>>) ->
    HexBin = binary_part(Bin, 0, byte_size(Bin) - 1),
    bin_to_hex:hex_to_bin(HexBin);
